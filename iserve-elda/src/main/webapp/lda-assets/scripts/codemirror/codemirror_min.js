var CodeMirrorConfig = window.CodeMirrorConfig || {}, CodeMirror = function () {
    function D(a, b) {
    ...
    function s(a) {
        return document.createElementNS && document.documentElement.namespaceURI !== null ? document.createElementNS("http://www.w3.org/1999/xhtml", a) : document.createElement(a)
    }
    ...
    function F(a, b) {
        ...
        c.style.height = "100%";
        if (c.style.setExpression)try {
            c.style.setExpression("height",
        ...
        a.appendChild(c);
        e.className = "CodeMirror-line-numbers";
        c.appendChild(e);
    ...
    function G(a) {
        ...
        E(a.basefiles.concat(a.parserfile), function (c) {
            ...
            b.push('<script type="text/javascript" src="' + c + (a.noScriptCaching ? "?nocache=" + (new Date).getTime().toString(16) : "") + '"><\/script>')
        });
    ...
    function t(a, b) {
        ...
        if (b.dumbTabs)b.tabMode = "spaces"; else if (b.normalTab)b.tabMode = "default";
        if (b.cursorActivity)b.onCursorActivity = b.cursorActivity;
        var c = this.frame = s("iframe");
        ...
        var e = this.wrapping = s("div");
        e.style.position = "relative";
        e.className = "CodeMirror-wrapping";
        e.style.width = b.width;
        e.style.height =
                b.height == "dynamic" ? b.minHeight + "px" : b.height;
        var h = this.textareaHack = s("textarea");
        e.appendChild(h);
        ...
        a.appendChild ? a.appendChild(e) : a(e);
        e.appendChild(c);
        if (b.lineNumbers)this.lineNumbers =
            F(e, b.firstLineNumber);
    ...
    t.prototype = {init: function () {
        ...
        clearHistory: function () {
            ...
            for (var b = {}, c = {}, e = this.win.document.getElementsByTagName("link"), h = 0, d; d = e[h]; h++)if (d.rel.indexOf("stylesheet") !== -1)for (var f = 0; f < a.length; f++) {
                var n = a[f];
                if (d.href.substring(d.href.length -
                    n.length) === n) {
                    b[d.href] = true;
                    c[n] = true
                }
            }
            for (h = 0; d = e[h]; h++)if (d.rel.indexOf("stylesheet") !== -1)d.disabled = !(d.href in b);
            ...
            if (a != this.options.textWrapping) {
                this.win.document.body.style.whiteSpace = a ? "" : "nowrap";
            ...
            if (a && !this.lineNumbers) {
                this.lineNumbers = F(this.wrapping, this.options.firstLineNumber);
            ...
            this.editor.insertIntoLine(a,
                b, c)
        }, selectLines: function (a, b, c, e) {
            this.win.focus();
            this.editor.selectLines(a, b, c, e)
        }, nthLine: function (a) {
        ...
        activateLineNumbers: function () {
            function a() {
                if (d.offsetWidth != 0) {
                    for (var g = d; g.parentNode; g = g.parentNode);
                    if (!i.parentNode || g != document || !f.Editor) {
                        try {
                            y()
                        } catch (k) {
                        }
                        clearInterval(J)
                    } else if (i.offsetWidth != z) {
                        z = i.offsetWidth;
            ...
            function b() {
                i.scrollTop = q.scrollTop || n.documentElement.scrollTop || 0
            }

            function c(g) {
                var k = l.firstChild.offsetHeight;
                if (k != 0) {
                    k = Math.ceil((50 + Math.max(q.offsetHeight, Math.max(d.offsetHeight, q.scrollHeight || 0))) / k);
                    for (var o = l.childNodes.length; o <=
                        k; o++) {
                        var w = s("div");
                        w.appendChild(document.createTextNode(g ? String(o + j.options.firstLineNumber) : "\u00a0"));
                        l.appendChild(w)
                    }
                }
            }
            ...
            function e() {
                ...
                j.updateNumbers = g;
                var k = f.addEventHandler(f, "scroll", b, true), o = f.addEventHandler(f, "resize", g, true);
                y = function () {
                    k();
                    o();
                    if (j.updateNumbers == g)j.updateNumbers = null
                };
                g()
            }
            ...
            function h() {
                function g(p, A) {
                    r || (r = l.appendChild(s("div")));
                    I && I(r, A, p);
                    u.push(r);
                    u.push(p);
                    B = r.offsetHeight + r.offsetTop;
                    r = r.nextSibling
                }

                function k() {
                    for (var p = 0; p < u.length; p +=
                        2)u[p].innerHTML = u[p + 1];
                    u = []
                }

                function o() {
                    if (!(!l.parentNode || l.parentNode != j.lineNumbers)) {
                        for (var p = (new Date).getTime() + j.options.lineNumberTime; m;) {
                            for (g(C++, m.previousSibling); m && !f.isBR(m); m = m.nextSibling)for (var A = m.offsetTop + m.offsetHeight; l.offsetHeight && A - 3 > B;)g("&nbsp;");
                            if (m)m = m.nextSibling;
                            if ((new Date).getTime() > p) {
                                k();
                                v = setTimeout(o, j.options.lineNumberDelay);
                                return
                            }
                        }
                        for (; r;)g(C++);
                ...
                function x() {
                    v &&
                    clearTimeout(v);
                    if (j.editor.allClean())w(); else v = setTimeout(x, 200)
                }

                var m, r, C, B, u = [], I = j.options.styleNumbers;
                w(true);
                var v = null;
                j.updateNumbers = x;
                var K = f.addEventHandler(f, "scroll", b, true), L = f.addEventHandler(f, "resize", x, true);
                y = function () {
                    v && clearTimeout(v);
                    if (j.updateNumbers == x)j.updateNumbers = null;
                    K();
                    L()
                }
            }

            var d = this.frame, f = d.contentWindow, n = f.document, q = n.body, i = this.lineNumbers, l = i.firstChild, j = this, z = null;
            i.onclick = function (g) {
                var k = j.options.onLineNumberClick;
                if (k) {
                    g = (g || event).target ||
                        (g || event).srcElement;
                    var o = g == i ? NaN : Number(g.innerHTML);
                    isNaN(o) || k(o, g)
                }
            };
            var y = function () {
            ...
            a();
            var J = setInterval(a, 500);
            (this.options.textWrapping || this.options.styleNumbers ? h : e)()
        }, setDynamicHeight: function () {
            function a() {
                for (var q = 0, i = h.lastChild, l; i && e.isBR(i);) {
                    i.hackBR || q++;
                    i = i.previousSibling
                }
                if (i) {
                ...
                if (l)b.wrapping.style.height = Math.max(n + l, b.options.minHeight) + "px"
            }

            var b = this, c = b.options.onCursorActivity, e = b.win, h = e.document.body, d =
                null, f = null, n = 2 * b.frame.offsetTop;
            h.style.overflowY = "hidden";
            e.document.documentElement.style.overflowY = "hidden";
            this.frame.scrolling = "no";
            setTimeout(a, 300);
            b.options.onCursorActivity = function (q) {
                c && c(q);
                clearTimeout(f);
                f = setTimeout(a, 100)
            }
        }};
    t.InvalidLineHandle = {toString: function () {
    ...
    t.fromTextArea = function (a, b) {
        ...
        if (a.form) {
            ...
            var e = a.form.submit, h = function () {
                c();
                a.form.submit = e;
                a.form.submit();
                a.form.submit = h
        ...
        var d = new t(function (f) {
            a.nextSibling ?
                a.parentNode.insertBefore(f, a.nextSibling) : a.parentNode.appendChild(f)
        }, b);
    ...
    t.isProbablySupported = function () {
        var a;
        return window.opera ? Number(window.opera.version()) >= 9.52 : /Apple Computer, Inc/.test(navigator.vendor) && (a = navigator.userAgent.match(/Version\/(\d+(?:\.\d+)?)\./)) ?
            Number(a[1]) >= 3 : document.selection && window.ActiveXObject && (a = navigator.userAgent.match(/MSIE (\d+(?:\.\d*)?)\b/)) ? Number(a[1]) >= 6 : (a = navigator.userAgent.match(/gecko\/(\d{8})/i)) ? Number(a[1]) >= 20050901 : (a = navigator.userAgent.match(/AppleWebKit\/(\d+)/)) ? Number(a[1]) >= 525 : null
    };
    return t
}();