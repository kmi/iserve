            ...
            <xsl:if test="$showMap = 'true'">
                ...
                <xsl:variable name="longParam">
                    <xsl:choose>
                        ...
                        <xsl:otherwise>
                            <xsl:value-of select="concat($mapParam, '.', $long)"/>
                        </xsl:otherwise>
                    </xsl:choose>
                </xsl:variable>
                <xsl:variable name="latParam">
                    <xsl:choose>
                        ...
                        <xsl:otherwise>
                            <xsl:value-of select="concat($mapParam, '.', $lat)"/>
                        </xsl:otherwise>
                    </xsl:choose>
                </xsl:variable>
                <!-- 
        ...
        <xsl:choose>
            <xsl:when test="$showMap = 'true'">
                <xsl:variable name="markers"
                              select="items//*[*[name(.) = $long] and *[name(.) = $lat]] | primaryTopic[not(../items)]/descendant-or-self::*[*[name(.) = $long] and *[name(.) = $lat]]"/>
                        ...
                        <xsl:choose>
                            <xsl:when test="$multipleMarkers">
                                ...
                                var center = new OpenLayers.LonLat(<xsl:value-of
                                    ...
                                    select="$minLat + (($maxLat - $minLat) div 2)"/>).transform(wgs84,summaryMap.getProjectionObject());
...
-->
                        ...
                        <xsl:for-each select="$markers">
                            ...
                            osMap = new OpenLayers.Map('<xsl:value-of select="concat('map', generate-id(.))"/>',
                            {controls: controls});
                            osMap.addLayer(new OpenLayers.Layer.OSM.Mapnik());
                            var center = new OpenLayers.LonLat(<xsl:value-of select="*[name(.) = $long]"/>,
                ...
                <xsl:choose>
                    <xsl:when test="$orderBy != ''">
                        <xsl:variable name="description">
                            <xsl:choose>
                                <xsl:when
                                        test="starts-with($orderBy, concat('(((?', $long, ' - ')) or starts-with($orderBy, concat('desc(((?', $long, ' - '))">
        ...
        <xsl:if test="name() != $bestLabelParam">
            ...
            <tr class="{name(.)}">
                ...
                <xsl:choose>
                    ...
                    <xsl:when
                            test="(name(.) = $lat or name(.) = $northing or name(.) = $easting) and $showMap = 'true'">
                        <td class="value">
                            <xsl:apply-templates select="." mode="display"/>
                        </td>
                    </xsl:when>
