...
-->
    ...
    <xsl:template match="result" mode="script">
        ...
        <xsl:if test="$showMap = 'true'">
            ...
            <script type="text/javascript"
                    src="http://ondemandapi.ordnancesurvey.co.uk/osmapapi/openspace.js?key=wec5muru"></script>
        </xsl:if>
        ...
        <script type="text/javascript">
            ...
            <xsl:if test="$showMap = 'true'">
                ...
                <xsl:variable name="eastingParam">
                    <xsl:choose>
                        <xsl:when test="$mapParam=''">
                            <xsl:value-of select="$easting"/>
                        </xsl:when>
                        <xsl:otherwise>
                            <xsl:value-of select="concat($mapParam, '.', $easting)"/>
                        </xsl:otherwise>
                    </xsl:choose>
                </xsl:variable>
                <xsl:variable name="northingParam">
                    <xsl:choose>
                        ...
                        <xsl:otherwise>
                            <xsl:value-of select="concat($mapParam, '.', $northing)"/>
                        </xsl:otherwise>
                    </xsl:choose>
                </xsl:variable>
                <!-- skw
                ...
                <xsl:variable name="properties">
                    <xsl:if test="not(/result/items)">_properties=<xsl:value-of select="$eastingParam"/>,<xsl:value-of
                            select="$northingParam"/>&amp;
                    </xsl:if>
                </xsl:variable>
                initMap();
                ...
                var orderBy =
                <xsl:if test="not(contains($uri, '_sort='))">(maxEasting - minEasting) &lt; 2000 ? '&amp;_orderBy=(((?<xsl:value-of
                        select="$easting"/> - ' + midEasting + ')*(?<xsl:value-of select="$easting"/>- ' + midEasting +
                    '))%2B((?<xsl:value-of select="$northing"/> - ' + midNorthing + ')*(?<xsl:value-of
                            select="$northing"/> - ' + midNorthing + ')))' :
                </xsl:if>
                '';
                ...
                <xsl:variable name="windowUri">
                    <xsl:call-template name="substituteParam">
                        ...
                        <xsl:with-param name="param">min-<xsl:value-of select="$eastingParam"/>
                        </xsl:with-param>
                        <xsl:with-param name="uri">
                            <xsl:call-template name="substituteParam">
                                <xsl:with-param name="value" select="''"/>
                                <xsl:with-param name="param">max-<xsl:value-of select="$eastingParam"/>
                                </xsl:with-param>
                                <xsl:with-param name="uri">
                                    <xsl:call-template name="substituteParam">
                                        <xsl:with-param name="value" select="''"/>
                                        <xsl:with-param name="param">min-<xsl:value-of select="$northingParam"/>
                                        </xsl:with-param>
                                        <xsl:with-param name="uri">
                                            <xsl:call-template name="substituteParam">
                                                <xsl:with-param name="value" select="''"/>
                                                <xsl:with-param name="param">max-<xsl:value-of select="$northingParam"/>
                                                </xsl:with-param>
                ...
                window.location = '<xsl:value-of select="concat($windowUri, $sep, $properties)"/>min-<xsl:value-of
                    select="$eastingParam"/>=' + minEasting + '&amp;max-<xsl:value-of select="$eastingParam"/>=' +
                maxEasting + '&amp;min-<xsl:value-of select="$northingParam"/>=' + minNorthing + '&amp;max-<xsl:value-of
                    select="$northingParam"/>=' + maxNorthing + orderBy;
                });
            </xsl:if>
            });
        </script>
    ...
    <xsl:template match="query" mode="skipLines">
        ...
        <xsl:choose>
            <xsl:when
                    test="starts-with($firstLine, 'prefix ') or (normalize-space($query) != '' and normalize-space($firstLine) = '')">
                <xsl:apply-templates select="." mode="skipLines">
            ...
            <xsl:when test="$showMap = 'true'">
                ...
                <section class="map">
                    ...
                    <script type="text/javascript">
                        ...
                        //var options = {controls: controls, products: ["OV0", "OV1", "OV2", "MSR", "MS", "CS04",
                        "CS05", "VMDR", "VMD", "VMLR", "VML"], bgcolor: '0x9ecfff'};
                        var options = {controls: controls, products: ["OV0", "OV1", "OV2", "MSR", "MS", "CS04", "CS05",
                        "CS06", "CS07", "CS08", "CS09", "CS10" ], bgcolor: '0x9ecfff'};
                        ...
                        <xsl:choose>
                            <xsl:when test="$multipleMarkers">
                                var bounds = new OpenLayers.Bounds(<xsl:value-of select="$minEasting"/>, <xsl:value-of
                                    select="$minNorthing"/>, <xsl:value-of select="$maxEasting"/>, <xsl:value-of
                                    select="$maxNorthing"/>);
                                var zoom = Math.min(
                                <xsl:choose>
                                    <xsl:when test="/result/next">7</xsl:when>
                                    <xsl:otherwise>10</xsl:otherwise>
                                </xsl:choose>
                                , summaryMap.getZoomForExtent(bounds));
                                var center = new OpenSpace.MapPoint(<xsl:value-of
                                    select="$minEasting + floor(($maxEasting - $minEasting) div 2)"/>, <xsl:value-of
                                    select="$minNorthing + floor(($maxNorthing - $minNorthing) div 2)"/>);
                                summaryMap.setCenter(center, zoom);
                            </xsl:when>
                            <xsl:when test="$markers">
                                var center = new OpenSpace.MapPoint(<xsl:value-of
                                    select="$markers/*[name(.) = $easting]"/>, <xsl:value-of
                        ...
                        <xsl:for-each select="$markers">
                            ...
                            pos = new OpenSpace.MapPoint(<xsl:value-of select="*[name(.) = $easting]"/>, <xsl:value-of
                                select="*[name(.) = $northing]"/>);
                            marker = new OpenLayers.Marker(pos, icon.clone());
                            <xsl:if test="/result/items">
                                marker.events.on({
                                mouseover: function () {
                                info.setHTML('&lt;div class=\"mapInfo\">&lt;a href=\"#<xsl:value-of
                                    select="generate-id(.)"/>\">
                        ...
                        <xsl:for-each select="$markers">
                            ...
                            var options = {controls: controls, products: ["OV0", "OV1", "OV2", "MSR", "MS", "CS04",
                            "CS05", "CS06", "CS07", "CS08", "CS09", "CS10" ], bgcolor: '0x9ecfff'};
                            // osMap = new OpenSpace.Map('<xsl:value-of select="concat('map', generate-id(.))"/>',
                            {controls: controls});
                            osMap = new OpenSpace.Map('<xsl:value-of select="concat('map', generate-id(.))"/>',
                            options);
                            var center = new OpenSpace.MapPoint(<xsl:value-of select="*[name(.) = $easting]"/>,
                            <xsl:value-of select="*[name(.) = $northing]"/>);
                ...
                <xsl:if test="$showBarchart = 'true'">
                    ...
                    <tr>
                        ...
                        <td class="barchart" id="barchart{generate-id(.)}">
                            <script type="text/javascript">
                                $('#barchart<xsl:value-of select="generate-id(.)"/>').sparkline([<xsl:value-of
                                    select="$grouped"/>], { type: 'bar', barColor: '<xsl:value-of
                                    select="$graphColour"/>' });
                            </script>
                        </td>
                    </tr>
    ...
    <xsl:template match="result" mode="viewnav">
        ...
        <section class="view">
            ...
            <ul class="properties">
                ...
                <xsl:for-each
                        select="(items/item/* | primaryTopic[not(../items)]/*)[generate-id(key('properties', name(.))[1]) = generate-id(.)]">
