...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:key name="properties"
             match="/result/items/item/*/*/*/*/*/*/*/* | /result[not(items)]/primaryTopic/*/*/*/*/*/*/*/*"
    ...
    <xsl:key name="items"
             match="*[not(parent::result or self::item/parent::*[not(self::items)]/parent::result) and @href]"
             use="@href"/>
        ...
        <xsl:variable name="text">
            <xsl:choose>
                ...
                <xsl:when test="starts-with($label, 'unknown')">
                    unknown
                    <xsl:value-of select="substring-after($label, 'Z')"/>
                </xsl:when>
                <xsl:otherwise>
                    ...
                    <xsl:otherwise>
                        <xsl:call-template name="splitOnCapital">
                            ...
                            <xsl:with-param name="token"
                                            select="translate(substring($string, 1, 1), $uppercase, $lowercase)"/>
                        </xsl:call-template>
                    </xsl:otherwise>
                </xsl:choose>
            </xsl:when>
            ...
            <xsl:with-param name="value">
                <xsl:choose>
                    <xsl:when test="contains($uri, concat('&amp;', $param, '='))">
                        <xsl:value-of
                                select="substring-before(concat(substring-after($uri, concat('&amp;', $param, '=')), '&amp;'), '&amp;')"/>
                    </xsl:when>
                    <xsl:when test="contains($uri, concat('?', $param, '='))">
                        <xsl:value-of
                                select="substring-before(concat(substring-after($uri, concat('?', $param, '=')), '&amp;'), '&amp;')"/>
    ...
    <xsl:template match="*" name="isLabelParam" mode="isLabelParam">
        ...
        <xsl:choose>
            <xsl:when
                    test="$paramName = $label or $paramName = $prefLabel or $paramName = $altLabel or $paramName = $name or $paramName = $title">
                true
            </xsl:when>
    ...
    <xsl:template name="subPath">
        ...
        <xsl:value-of
                select="concat(substring-before($uri, $componentPart), $componentPart, substring-before(concat($after, '/'), '/'))"/>
    </xsl:template>
