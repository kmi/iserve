...
public class SchemaParser {
    ...
    public SchemaMap parse(Schema schema) throws URISyntaxException {
        org.w3c.dom.Element doc = schema.getElement();
        return parse(doc);
    }

    public SchemaMap parse(XmlSchema xmlSchema) throws URISyntaxException {
        ...
        for (Document schema : schemas) {
            if (schema != null && schema.getDocumentElement() != null) {
                result.addSchemaMap(parse(schema.getDocumentElement()));
            }
        }
        return result;
    }

    private SchemaMap parse(org.w3c.dom.Element xmlElement) throws URISyntaxException {
        ...
        for (int i = 0; i < nodes.getLength(); i++) {
            ...
            if (node != null && node.getLocalName() != null) {
                if (node.getLocalName().equalsIgnoreCase("simpleType")) {
                    SimpleType simpleType = new SimpleType(node);
