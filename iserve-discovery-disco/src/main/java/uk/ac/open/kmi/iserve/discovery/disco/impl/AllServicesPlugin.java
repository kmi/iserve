...
public class AllServicesPlugin implements ServiceDiscoveryPlugin, OperationDiscoveryPlugin {
	...
	public Map<URL, MatchResult> discover(boolean operationDiscovery, MultivaluedMap<String, String> parameters) throws DiscoveryException {
		...
		StringBuffer queryBuffer = new StringBuffer(Util.generateQueryPrefix());
		queryBuffer.append("select ?svc ?labelSvc ");
		if (operationDiscovery) {
			queryBuffer.append("?op ?labelOp");
		}
		queryBuffer.append(NEW_LINE);
		queryBuffer.append("where {  ?svc a msm:Service ." + NEW_LINE);

		if (operationDiscovery) {
			queryBuffer.append("?svc msm:hasOperation ?op ." + NEW_LINE);
			queryBuffer.append("?op a msm:Operation ." + NEW_LINE);
		} 
		queryBuffer.append("  optional { ?svc rdfs:label ?labelSvc }" + NEW_LINE);
		if (operationDiscovery) {
			queryBuffer.append("  optional { ?op rdfs:label ?labelOp }" + NEW_LINE);
		}
		queryBuffer.append("}");

		log.info("Querying the backend: " + sparqlEndpoint + NEW_LINE + queryBuffer);
